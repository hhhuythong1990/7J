...
public class ExcelManager {
	...
	public void zipfile_many(java.util.ArrayList arrFileName, String sPath,
			String OutZip) {
		...
		try {
			...
			for (int i = 0; i < sizefile; i++) {
				...
				in = new BufferedInputStream(new FileInputStream(sPath + "/"
						+ arrFileName.get(i)), 1000);
		...
		if (cell != null) {
			...
			case HSSFCell.CELL_TYPE_NUMERIC:
				...
				if (HSSFDateUtil.isCellDateFormatted(cell)) {
					...
					sValue = (String.valueOf(cal.get(Calendar.YEAR)))
							.substring(0);
					sValue = this.getHaiSo(String.valueOf(cal
							.get(Calendar.DAY_OF_MONTH)))
							+ "/"
							+ (cal.get(Calendar.MONTH) + 1) + "/" + sValue;
				} else {

					if (!String.valueOf(cell.getNumericCellValue()).equals("")
							&& !String.valueOf(cell.getNumericCellValue())
									.equals("null"))
						sValue = oDate.formatNum_Grid(cell
								.getNumericCellValue());
			...
			case HSSFCell.CELL_TYPE_FORMULA:
				...
				if (cellValue != null) {
					...
					case HSSFCell.CELL_TYPE_BOOLEAN:
						...
						break;
					case HSSFCell.CELL_TYPE_NUMERIC:
						sValue = oDate.formatNum_Grid(cellValue
								.getNumberValue());
						break;
					case HSSFCell.CELL_TYPE_FORMULA:
						sValue = oDate.formatNum_Grid(cellValue
	...
	@SuppressWarnings({ "static-access", "deprecation" })
	public String getValueExcel_NoNumber(HSSFCell cell, HSSFSheet sheet,
			HSSFWorkbook wb) {
		Utilities oDate = new Utilities();
